...
namespace Pop3 
{
    ...
    public class POP3 
    {
        ...
        protected void setPop3ConnectionState(Pop3ConnectionStateEnum State) 
        {
            ...
            CallTrace("   Nhom11Server Connection State {0} reached" + CRLF, State );
        }
        #endregion
      ...
      if (pop3ConnectionState!=requiredState) {
        ...
        /// </summary>
        public NetworkStream pop3Stream;
        /// <summary>
        /// Reader for POP3 message
        /// </summary>
        public StreamReader pop3StreamReader;
        /// <summary>
        ...
        /// </summary>
        /// <param name="PopServer"><example>pop.Nhom11Server.nt106</example></param>
        ...
        public bool Connect()
        {
            ...
            else
            {
                ...
                try
                {
                    CallTrace("   Connect at port {0}", 8080 );
                    serverTcpConnection = new TcpClient("127.0.0.1", 110);
                    CallTrace("Connecting ", " to MyServer at 127.0.0.1");
                }
                catch (Exception ex)
                {
                ...
                try
                {
                    pop3StreamReader = new StreamReader(pop3Stream);
                }
                catch (Exception ex)
                {
        ...
        public void Disconnect() 
        {
            ...
            {
                TraceFrom(Pop3ConnectionState.ToString(), "Disconnect received, but was already disconnected.");
            } 
            else 
            {
        ...
        /// </summary>
        /// <param name="MessageNo">ID of message required</parCam>
        /// <returns>false: negative server respond, message not delivered</returns>
        protected bool SendRetrCommand(int MessageNo,out string response)
        {
            EnsureState(Pop3ConnectionStateEnum.Connected);
        ...
        /// <returns></returns>
        public bool GetRawEmail(int MessageNo, out string response)
        {
            //send 'RETR int' command to server
            if (!SendRetrCommand(MessageNo,out response))
            {
                return false;
            }
            //get the lines
        ...
        public int GetEmailSize(int msg_number) 
        {
            ...
            //TraceFrom(responseSplit[0], responseSplit[1]);
        ...
        private bool executeCommand(string command, out string response)
        {
            ...
            CallTrace("Client:  {0} ", command + CRLF);
            ...
            try
            {
                ...
                if (pop3Stream.CanRead)
                {
                    var recv = pop3Stream.Read(data, 0, data.Length);
        ...
        public bool CreateNewAccount()
        {
            EnsureState(Pop3ConnectionStateEnum.Connected);
            string response;
            if (!executeCommand("CRTU " + username + " " + password, out response))
            {
                CallTrace(response);
                return false;
            }
